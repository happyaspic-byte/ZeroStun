...
- `tests/lvm_contract.rs`: LVM JSON probe, target validation, read-only tagged
  ...
  timeout, cancellation, redaction, and tampered handle rejection.
- `tests/zfs_contract.rs`: machine-readable probe, filesystem/ZVOL capability
  ...
  timeout, cancellation, redaction, and handle validation.
...
| --- | --- | --- | --- |
| LVM | `/usr/sbin/lvs --reportformat json --options vg_name,lv_name,lv_tags` | Binary present; host access is non-root and LVM lock access is denied, so no disposable integration environment is available | None |
...
- LVM accepts only exact validated `vg/lv` targets. Cleanup requires both a
  `zerostun-` identifier and the `zerostun.snapshot` LVM tag.
...
- Recovery lists only resources whose identifiers and ownership relationships
  ...
  order. Unmanaged resources are ignored.
...
| `cargo test --lib --bins --tests --all-features --offline -- --test-threads=1` | PASS: 162 passed, 0 failed, 0 ignored across 14 suites (including `lvm_contract` and `zfs_contract`) |
